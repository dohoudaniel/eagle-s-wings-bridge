...
function AboutPage() {
  const { data: timeline, loading } = useApi(api.getTimeline);
        ...
        <div className="container mx-auto px-4 lg:px-8 grid lg:grid-cols-2 gap-12 lg:gap-20 items-start">
          <div>
            ...
            <p className="mt-5 text-lg text-muted-foreground leading-relaxed">
              We serve vulnerable children, the elderly in need of care, and women & youth seeking
              opportunity. Every program is rooted in long-term outcomes, not short-term aid.
            </p>
          </div>
          <div>
            <div className="text-xs uppercase tracking-[0.2em] font-semibold text-primary mb-3">
            ...
            <p className="mt-5 text-lg text-muted-foreground leading-relaxed">
              Connecting Europe's generosity with Nigeria's needs — transparently, accountably, and
            ...
            {loading || !timeline
              ? Array.from({ length: 5 }).map((_, i) => (
                  <div key={i} className="relative pl-14">
                    <div className="absolute left-0 top-1 h-8 w-8 rounded-full bg-muted" />
                    <div className="animate-pulse h-4 w-16 bg-muted rounded mb-2" />
                    <div className="animate-pulse h-6 w-2/3 bg-muted rounded mb-2" />
